 ...
 * - game_id   = OR2
 * - main balls = 4 (first, second, third, fourth)
 * - main range = 01..77
...
if (isset($stateName, $gName) && $stateName !== '' && $gName !== '') {
    ...
    $metaDesc = 'Detailed draw frequency, recency, and number behavior analysis for '
        . $stateName . ' ' . $gName
        . '. Review active and quiet numbers, historical patterns, and draw context at LottoExpert.net.';
...
function leGetPreviousOccurrenceDate(
    ...
    $query = $db->getQuery(true)
        ...
        ->where(
            ...
            $db->quoteName('fourth') . ' = ' . $db->quote($ball) .
            ')'
        );
...
[$mainCounts, $mainLastSeenIndex] = leInitRange(1, 77);
 ...
 * --------------------------------------------------------------------------
 */
$mainChartLabels  = leBuildNaturalLabels(1, 77);
$mainChartValues  = [];
$mainRecencyValues = [];
...
[$counts50,  ] = leInitRange(1, 77);
[$counts300, ] = leInitRange(1, 77);

foreach ($window50 as $row) {
    foreach (['first', 'second', 'third', 'fourth'] as $col) {
...
/**
 * --------------------------------------------------------------------------
 * Draw history summary for current draw (main balls only, no bonus)
 * --------------------------------------------------------------------------
 */
$drawHistoryRows = [];
...
$jsonLdData = [
    ...
    '@type'       => 'WebPage',
    'name'        => (isset($stateName, $gName) ? $stateName . ' ' . $gName . ' Results Analysis | LottoExpert.net' : 'Lottery Results Analysis | LottoExpert.net'),
    'description' => (isset($metaDesc) ? $metaDesc : 'Lottery number frequency and draw history analysis at LottoExpert.net.'),
...
$jsonLdOutput = json_encode($jsonLdData, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
$doc->addCustomTag('<script type="application/ld+json">' . $jsonLdOutput . '</script>');
?>
<style>
:root{
  --skai-blue:#1C66FF;
      ...
      <div class="skai-hero-top">
        ...
        <div class="skai-hero-copy">
          ...
          <div class="skai-hero-actions" aria-label="Primary actions">
            <a class="skai-btn skai-btn--primary" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=OR2">
              Open SKAI Analysis
            </a>
            <a class="skai-btn skai-btn--secondary" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=OR2">
              AI Predictions
            </a>
          ...
          <div class="skai-advanced-links" aria-label="Advanced tools">
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=OR2">Skip &amp; Hit Analysis</a>
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?game_id=OR2">MCMC Markov Analysis</a>
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/oregon-win-for-life-heatmap-analysis">Heatmap Analysis</a>
          </div>
        </div>
          ...
          <div class="skai-card-body">
            <div class="skai-history-list">
              <?php if (!empty($drawHistoryRows)) : ?>
                ...
                <?php endforeach; ?>
              <?php else : ?>
                <div class="skai-note">No draw data available for the current record.</div>
              <?php endif; ?>
            </div>
          </div>
        </div>
  ...
  <section id="frequency-deep-dive" class="skai-section" aria-labelledby="frequency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Move from summary to full reference. The panel below shows the complete main-number distribution across all values 01&ndash;77. The recency distribution shows how long ago each number last appeared within the current window.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            ...
            <span class="skai-card-sub">All values 01&ndash;77 across the last <?php echo (int) $nodCurrentMain; ?> drawings</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-chart-shell">
      ...
      <div class="skai-note">
        The complete distribution is best used as a reference layer. The overview modules above are optimized for quick reading; this section is optimized for thorough review. Numbers absent from the current window appear at the maximum recency distance.
      </div>
    </div>
  </section>
  ...
  <section id="tables" class="skai-section" aria-labelledby="tables-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Use the table for exact counts, recency, and personal tracking. Quick filters help narrow the view without losing full access to the complete dataset. Adjust the draw window below to reload analysis for a different number of past drawings.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-card">
        <div class="skai-card-head skai-card-head--horizon">
          Main numbers table
          <span class="skai-card-sub">Exact counts and recency for values 01&ndash;77 across the last <?php echo (int) $nodCurrentMain; ?> drawings</span>
        </div>
      ...
      <div class="skai-tool-grid">
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=OR2">Open SKAI Analysis</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=OR2">Open AI Predictions</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=OR2">Open Skip &amp; Hit</a>
          </div>
        </article>
      </div>

      <div class="skai-utility-grid">
        <a class="skai-utility-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?game_id=OR2">MCMC Markov Analysis</a>
        <a class="skai-utility-link" href="/picking-winning-numbers/artificial-intelligence/oregon-win-for-life-heatmap-analysis">Heatmap Analysis</a>
